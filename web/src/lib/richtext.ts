...
const BLOCK_BREAK_PATTERN = /<\/(p|div|li|ul|ol|blockquote|h[1-6]|table|thead|tbody|tr)>/gi;

export const renderRichTextPlain = (input: string): string => {
  const html = renderRichText(input);
  if (!html) {
    return '';
  }

  const withBreaks = stripDangerousTags(html)
    .replace(/<br\s*\/?\s*>/gi, '\n')
